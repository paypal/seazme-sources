...
(comment
  ...
  (defn upload-period-2step[{:keys [kind instance index]} cached? pja-search pja-issue-full f period]
    (let [period2 (map co/to-date-time period)
          period3 (map (partial tf/unparse ff2) period2)
          _ (print "downloading (2step)JIRAs for:" period3 cached? kind instance index)
          _ (flush)
  ...
  (defn upload-period-nonstreamed[{:keys [kind instance index]} cached? dry-run pja-search f period]
    (try
      (let [period2 (map co/to-date-time period)
            period3 (map (partial tf/unparse ff2) period2)
            _ (print "downloading JIRAs (1step) for:" period3 cached? kind instance index)
            _ (flush)
            base-path (format "db/%s-cache/%s/%s" kind instance index)
            _ (fs/mkdirs base-path)
            file-path (apply format "%s/%s-%s.edn" base-path (map (partial tf/unparse ff3) period2))
...
(defn upload-period[{:keys [kind instance index]} cached? dry-run pja-search f period]
  (try
    (let [period2 (map co/to-date-time period)
          period3 (map (partial tf/unparse ff2) period2)
          _ (print "downloading JIRAs (1step with 1/s stream) for:" period3 cached? kind instance index)
          _ (flush)
          base-path (format "db/%s-cache/%s/%s" kind instance index)
          _ (fs/mkdirs base-path)
...
(defn upload-period2[{:keys [kind instance index]} cached? dry-run pja-search f period]
  (try
    (let [period2 (map co/to-date-time period)
          period3 (map (partial tf/unparse ff2) period2)
          _ (print "downloading JIRAs (1step full stream) for:" period3 cached? kind instance index)
          _ (flush)
          base-path (format "db/%s-cache/%s/%s" kind instance index)
          _ (fs/mkdirs base-path)
          file-path (apply format "%s/%s-%s.edn.gz" base-path (map (partial tf/unparse ff3) period2))
