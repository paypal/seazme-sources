...
(defn parse-page [{:keys [base-url instance-name content]}]
  (let [p content
    ...
    {:id (format "%s/%s/%s/%s" instance-name confluence-type-name space-id page-id)
     ...
     :last-ts (-> (->> content :version :when (tf/parse conf-ts-formatter) tc/to-long) (quot 1000))
     :text text
...
(defn- find-updated-pages-between [api mins-from mins-to]
  (let [from (. f2 format mins-from)
        ...
        url (format "/rest/api/content/search?cql=(lastModified >= \"%s\" AND lastModified < \"%s\")&limit=300" from to)] ;;TODO returns 200 and empty if not logged in
...
(defn pull-confl-incr [api db-path]
  (let [inv-f (fn[path from to]
                (let [edits (find-updated-pages-between api from to)
                  ...
                  (->> page-edits (map #(save-page-from-search path api %)) dorun)
